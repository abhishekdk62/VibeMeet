  ...
  implements OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleHostMuteParticipant(
    ...
    try {
      ...
      this.logger.log(
        `Host ${socket.id} muted participant ${data.targetSocketId}`,
      );
    } catch (error) {
  ...
  async handleHostUnmuteParticipant(
    ...
    try {
      ...
      this.logger.log(
        `Host ${socket.id} unmuted participant ${data.targetSocketId}`,
      );
    } catch (error) {
  ...
  async handleHostDisableVideo(
    ...
    try {
      ...
      this.logger.log(
        `Host ${socket.id} disabled video for participant ${data.targetSocketId}`,
      );
    } catch (error) {
  ...
  async handleHostEnableVideo(
    ...
    try {
      ...
      this.logger.log(
        `Host ${socket.id} enabled video for participant ${data.targetSocketId}`,
      );
    } catch (error) {
  ...
  async handleRemoveParticipant(
    ...
    try {
      ...
      this.logger.log(
        `Host ${socket.id} removed participant ${data.targetSocketId}`,
      );
    } catch (error) {
  ...
  async handleJoinMeeting(
    ...
    try {
      ...
      this.logger.log(
        `Sockets in room ${meetingId}:`,
        Array.from(this.meetingRooms.get(meetingId) || []),
      );
      ...
      if (participants.length === 1) {
        this.logger.log(
          'First participant - updating meeting status to active',
        );
      ...
      try {
        ...
        this.logger.warn(
          'Database update failed, but continuing:',
          dbError.message,
        );
      }
      ...
      this.logger.log(
        `✅ User ${userName} successfully joined meeting ${meetingId} as ${isActualHost ? 'host' : 'participant'}`,
      );
  ...
  handleSendMessage(
    ...
    this.logger.log(
      `✅ Message sent in meeting ${data.meetingId} by ${user.userName}`,
    );
  }
  ...
  private getMeetingParticipants(meetingId: string): MeetingParticipant[] {
    ...
    console.log(
      `Socket IDs in room:`,
      socketIds ? Array.from(socketIds) : 'No room found',
    );
